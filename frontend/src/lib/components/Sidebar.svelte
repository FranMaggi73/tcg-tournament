<script lang="ts">
	...
	import { invitationCountStore, refreshInvitationCount } from '$lib/stores/notifications.svelte';
	...
	const navItems = [
		...
		{ href: '/profile', label: 'Perfil', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', showBadge: true }
	];
...
<aside class="fixed top-0 left-0 h-screen bg-base-200 border-r border-base-300 z-40 flex flex-col transition-all duration-200 {collapsed ? 'w-16' : 'w-52'}">
	...
	<nav class="flex-1 py-4 space-y-1 px-2">
		{#each navItems as item}
			<a
				href={item.href}
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors relative {isActive(item.href, $page.url.pathname) ? 'bg-primary text-primary-content' : 'text-base-content/70 hover:bg-base-300 hover:text-base-content'}"
			...
			>
				...
				{/if}
				{#if item.showBadge && invitationCountStore.count > 0}
					<span class="absolute top-1 {collapsed ? 'right-1' : 'right-2'} badge badge-primary badge-xs font-bold">
						{invitationCountStore.count > 9 ? '9+' : invitationCountStore.count}
					</span>
				{/if}
			</a>
		{/each}
